  ...
  if (checkedExt) {
    ...
    reader.onload = () => {
      ...
      const newData: KeyIndex<string>[] = data.map((item: any) => {
        const requiredFields: string[] = ['UTM 파라미터', '파라미터 값', '설명'];
...
const utmSample: KeyIndex<string>[] = [{
  'UTM 파라미터': 'UTM 파라미터 입력',
  '파라미터 값': '파라미터 값 입력',
  '설명': '설명 입력',
}];
/**
...
const downloadForm = (): void => {
  const ws: WorkSheet = XLSX.utils.json_to_sheet(utmSample);
  ...
  XLSX.utils.book_append_sheet(wb, ws, 'utm_sample');

  writeFileXLSX(wb, 'utm_sample.xlsx');
};
</script>
...
<template>
  ...
  >
    ...
    <template #action="{ close }">
      ...
      <StyledButton color="primary" :disabled="!isCheckedFile" @click="openResult">
        <SvgIcon
          class="loading"
          type="mdi"
