...
import type { RollingBannerSectionConfig } from '@/types/api/smartEditorApi';
...
const { PAGE_SECTION_TYPE: { ROLLING_BANNER } } = CONST_CODES;
...
const config = reactive<RollingBannerSectionConfig>({
  sectionRollingBannerUid: '',
  pageSectionRelUid: '',
...
const save = (close: Function): void => {
  if (form.value?.validate()) {
    if (props.index > -1) {
      ...
      setSection(ROLLING_BANNER.VAL, config);
    }
...
const sectionTitle = computed<string>(() => '롤링 배너 ' + (props.index > -1 ? '수정' : '등록'));

if (props.index > -1) {
  const data = getSection<RollingBannerSectionConfig>(props.index);
  ...
  if (!data) {
    ...
    config.sectionRollingBannerUid = data.config.sectionRollingBannerUid;
    config.pageSectionRelUid = data.config.pageSectionRelUid;
    config.compRollingBannerGrpUid = data.config.compRollingBannerGrpUid;
        ...
        <div class="row">
          <div class="col">
            <SelectBox
              ...
              label="배너 그룹명"
              placeholder="선택"
              :options="options.rollingBanner"
              :validate="rules.select"
              @blur="validation"
              @observe="optionCall('rollingBanner')"
              v-model="config.compRollingBannerGrpUid"
            />
          </div>
        </div>
        <div class="row pt-15">
