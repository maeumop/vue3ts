...
import type { GetImportsParam, GetImportsRes, ImportItem } from '@/types/api/smartEditorApi';
...
import GroupEditor from '@/views/smart/component/Import/layer/GroupEditor/index.vue';
...
const listTable = useListTable<ImportItem>({ headers: _tableHeader });
...
// copyComponent : 복사 팝업
// preview : 미리보기 팝업
// groupEditor : 임포트 등록/수정 팝업
type ModalType =  {
  copyComponent: SmartCompUseModal,
  preview: SmartCompUseModal<string>,
  groupEditor: SmartCompUseModal<ImportItem>,
};
...
// computed
const tableItems = computed<ImportItem[]>(() => toValue(listTable.items));

// methods
const { deleteSmartComponent } = useMessageBox(Toast);

const _getImports = async (): Promise<ImportItem[]> => {
  const req: GetImportsParam = { search: searchForm.getSearchParams(), limit: 50 };
  let cursor: number = -1;
  ...
  const { totalCount, imports } = res.results;
...
const onObserveTable = async (): Promise<void> => {
  try {
    ...
    await listTable.onObserve(() => _getImports());
  } catch (err) {
    util.axiosErrorCatch<GetImportsRes>(err);
  } finally {
 ...
 * @returns
 */
const onClickCopyBtn = async (item: ImportItem): Promise<void> => {
  if (loading.value) {
    return;
  }
  ...
  try {
    ...
    copyComponentEL.value?.onOpen({ self: item,  type: COPY_MODE.IMPORT });
  } catch (error) {
    onPopupClose(_key);
  }
};
...
const onCopyComponent = (item: SmartComponent): void => {
  if (!searchText.value) {
    ...
    tableItems.value.splice(0, 0, item as ImportItem);
    return;
  }
...
const onClickDeleteBtn = async (index: number): Promise<void> => {
  ...
  deleteSmartComponent('임포트', async () => {
    await smartEditorApi.deleteImports(deleteItem.compImportUid)
      .then(() => {
...
/**
 * 임포트 등록/수정 클릭 이벤트 핸들러
 * @param target? {ImportItem} - 수정시, 수정 정보
 * @returns
 */
const onOpenGroupEditor = async (target?: ImportItem): Promise<void> => {
  if (loading.value) {
    return;
  }
...
/**
 * 임포트 등록 완료 이벤트 핸들러
 * @param item {ImportItem} - 등록할 정보
 * @returns
 */
const onCreateComponent = (item: ImportItem): void => {
  if (!searchText.value) {
    tableTotalCount.value++;
...
/**
 * 임포트 수정 완료 이벤트 핸들러
 * @param edit {ImportItem} - 수정할 정보
 * @returns
 */
const onEditComponent = (edit: ImportItem): void => {
  if (!searchText.value) {
    const _key: ModalKeys = 'groupEditor';
...
/**
 * 임포트 미리보기
 * @param target {ImportItem}
 * @returns
 */
const onClickPreview = (target: ImportItem): void => {
  const _key: ModalKeys = 'preview';

  Spinner.delay(0).show('Loading…');
  modal[_key].show = true;
  modal[_key].target = target.compImportUid;
};
...
<template>
  <div id="smartImport" class="content common-list">
    ...
    >
      <template #items="{ props, index }: ListTableItemSlot<ImportItem>">
        <tr>
          <td>
            <StyledButton
              text
    ...
    />
    <Preview
      :component="previewComponent.IMPORT"
...
<script lang='ts'> export default { name: 'SmartComponentImport' };</script>