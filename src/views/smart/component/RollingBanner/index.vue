...
import type { GetRollingBannerGrpsParam, GetRollingBannerGrpsRes, RollingBannerGrpItem } from '@/types/api/smartEditorApi';
...
import BannerEditor from '@/views/smart/component/RollingBanner/layer/BannerEditor/index.vue';
import GroupEditor from '@/views/smart/component/RollingBanner/layer/GroupEditor/index.vue';
...
const bannerEditorEL = ref<Pick<InstanceType<typeof BannerEditor>, 'onOpen'| '$nextTick'>>();

// data()
const searchForm = useSearchForm([
  { text: '배너 그룹명', value: 'rollingBannerGrpName' },
...
const listTable = useListTable<RollingBannerGrpItem>({ headers: _tableHeader, observer: false });
...
// copyComponent : 복사 팝업
// preview : 미리보기 팝업
// groupEditor : 롤링배너 그룹 등록/수정 팝업
// bannerEditor : 롤링배너 설정 팝업
type ModalType =  {
  copyComponent: SmartCompUseModal,
  preview: SmartCompUseModal<string>,
  groupEditor: SmartCompUseModal<RollingBannerGrpItem>,
  bannerEditor: SmartCompUseModal,
};
...
// computed
const tableItems = computed<RollingBannerGrpItem[]>(() => toValue(listTable.items));

// methods
const { deleteSmartComponent } = useMessageBox(Toast);

const _getRollingListGrps = async (): Promise<RollingBannerGrpItem[]> => {
  const req: GetRollingBannerGrpsParam = { search: searchForm.getSearchParams(), limit: 50 };
  let cursor: number = -1;
  ...
  const { totalCount, rollingBannerGrps } = res.results;
  ...
  // observer 해제
  if (rollingBannerGrps.length < req.limit!) {
    await listTable.changeObserver(false);
  }

  return rollingBannerGrps;
};
...
const onObserveTable = async (): Promise<void> => {
  try {
    ...
    util.axiosErrorCatch<GetRollingBannerGrpsRes>(err);
  } finally {
 ...
 * @returns
 */
const onClickCopyBtn = async (item: RollingBannerGrpItem): Promise<void> => {
  if (loading.value) {
    return;
  }
  ...
  try {
    ...
    copyComponentEL.value?.onOpen({ self: item,  type: COPY_MODE.ROLLING_BANNER });
  } catch (error) {
    onPopupClose(_key);
  }
};
...
const onCopyComponent = (item: SmartComponent): void => {
  if (!searchText.value) {
    ...
    tableItems.value.splice(0, 0, item as RollingBannerGrpItem);
    return;
  }
...
const onClickDeleteBtn = async (index: number): Promise<void> => {
  ...
  deleteSmartComponent('배너 그룹', async () => {
    await smartEditorApi.deleteRollingBannerGrps(deleteItem.compRollingBannerGrpUid)
      .then(() => {
...
/**
 * 롤링배너 등록/수정 클릭 이벤트 핸들러
 * @param target? {RollingBannerGrpItem} - 수정시, 수정 정보
 * @returns
 */
const onOpenGroupEditor = async (target?: RollingBannerGrpItem): Promise<void> => {
  if (loading.value) {
    return;
  }
  ...
  try {
    ...
    editorEL.value?.onOpen(target ? { target } : {});
...
/**
 * 롤링배너 등록 완료 이벤트 핸들러
 * @param item {RollingBannerGrpItem} - 등록할 정보
 * @returns
 */
const onCreateComponent = async (item: RollingBannerGrpItem): Promise<void> => {
  if (!searchText.value) {
    tableTotalCount.value++;
...
/**
 * 롤링배너 수정 완료 이벤트 핸들러
 * @param edit {RollingBannerGrpItem} - 수정할 정보
 * @returns
 */
const onEditComponent = (edit: RollingBannerGrpItem): void => {
  if (!searchText.value) {
    const _key: ModalKeys = 'groupEditor';
...
/**
 * 배너 관리 모달 생성 이벤트 핸들러
 * @param target {RollingBannerGrpItem}
 * @returns
 */
const onClickBannerEditBtn =  async (target: RollingBannerGrpItem): Promise<void> => {
  if (loading.value) {
    return;
  }

  const _key: ModalKeys = 'bannerEditor';

  try {
    if (!modal[_key].show) {
      modal[_key].show = true;
      await bannerEditorEL.value?.$nextTick();
    }

    bannerEditorEL.value?.onOpen({ target });
  } catch (error) {
    onPopupClose(_key);
  }
};
 ...
 * @returns
 */
const onClickPreview = (target: RollingBannerGrpItem): void => {
  const _key: ModalKeys = 'preview';

  Spinner.delay(0).show('Loading…');
  modal[_key].show = true;
  modal[_key].target = target.compRollingBannerGrpUid;
};
...
<template>
  <div id="smartRollingBanner" class="content common-list">
    ...
    >
      <template #items="{ props, index }: ListTableItemSlot<RollingBannerGrpItem>">
        <tr>
          <td>
            <StyledButton
              text
    ...
    <BannerEditor
      ref="bannerEditorEL"
      @on:close="onPopupClose('bannerEditor')"
      v-if="modal['bannerEditor'].show"
    />
    <Preview
      :component="previewComponent.ROLLING_BANNER"
...
<script lang='ts'>
export default { name: 'SmartComponentRollingBanner' };
</script>